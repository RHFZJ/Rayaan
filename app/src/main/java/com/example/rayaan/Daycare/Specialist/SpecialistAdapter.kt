...
class SpecialistAdapter(private val list: ArrayList<DayCare>, private val context: Activity, private val a: Activity)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.daycare_specialist_recycler_row, parent, false)
        session = Session(a)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.add.setOnClickListener(View.OnClickListener {
            ...
            mDatabase!!.child("Users").child(session.getId().toString()).child("worker")
                .child(uniqueKey.toString()).child("id").setValue(list.get(position).id)
            Toast.makeText(context, "تم اضافة الموظف", Toast.LENGTH_SHORT).show()
